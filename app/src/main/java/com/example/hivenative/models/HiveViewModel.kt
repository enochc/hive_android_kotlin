...
class HiveViewModel(private val hiveName:String="Android Hive"): ViewModel(), LifecycleObserver {
    ...
    fun deleteProperty(name: String): Int {
        val index = hive.deleteProperty(name)
        return index
    }
    ...
    fun resumeHive() {
        ...
        job = viewModelScope.launch {
            withContext(Dispatchers.IO) {
                ...
                hive.peersChanged = {
                    viewModelScope.launch(Dispatchers.Main) {
                        peers.value = hive.peersList.filter {
                            it != hiveName
                        }
                    }
                }
